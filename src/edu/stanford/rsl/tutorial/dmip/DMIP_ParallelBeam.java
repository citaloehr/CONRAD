...
public class DMIP_ParallelBeam {
	...
	public Grid1D rampFiltering(Grid1D sinogram, RampFilterType filter){
		// grid1d sinogram is no sinogram.. its just one line
		double deltaS = 1;
		...
		else if(filter == RampFilterType.SHEPPLOGAN)
		{
			...
			ramp.setAtIndex(0, 2/ (float) Math.pow(Math.PI, 2));
			for(int i = 1; i < paddedSize/2; ++i) {
				
					ramp.setAtIndex(i, - 2.0f / (float) (Math.pow(Math.PI, 2) * ( 4* Math.pow(i, 2) - 1.0)));
			...
			for (int i = paddedSize/2 ; i < paddedSize; ++i) {
				final float tmp = paddedSize - i;
					...
					ramp.setAtIndex(i, - 2.0f / (float) (Math.pow(Math.PI, 2) * ( 4* Math.pow(tmp, 2) - 1.0)));
